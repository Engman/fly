...
bool PauseMenu::Initialize(FlyGame* entry, float windowHeight, float windowWidth)
{
	...
	if(!entry->GetCoreInstance()->Geometry_Load(L"..\\Resources\\Models\\dead_idle.fgm", &this->models))
		return false;
	this->models[6]->setScale(vec3(this->windowWidth*0.5f, this->windowHeight*0.5f, 1.0f));
	this->models[6]->setPosition(vec3(0.0f, 0.0f, 0.0f));
	this->models[6]->setShader(shaders[FlyShader_gBufferDefault]);
	this->models[6]->setBoundingSphere(sphere);
	if(!entry->GetCoreInstance()->Geometry_Load(L"..\\Resources\\Models\\dead_resume.fgm", &this->models))
		return false;
	this->models[7]->setScale(vec3(this->windowWidth*0.5f, this->windowHeight*0.5f, 1.0f));
	this->models[7]->setPosition(vec3(0.0f, 0.0f, 0.0f));
	this->models[7]->setShader(shaders[FlyShader_gBufferDefault]);
	this->models[7]->setBoundingSphere(sphere);
	if(!entry->GetCoreInstance()->Geometry_Load(L"..\\Resources\\Models\\dead_quit.fgm", &this->models))
		return false;
	this->models[8]->setScale(vec3(this->windowWidth*0.5f, this->windowHeight*0.5f, 1.0f));
	this->models[8]->setPosition(vec3(0.0f, 0.0f, 0.0f));
	this->models[8]->setShader(shaders[FlyShader_gBufferDefault]);
	this->models[8]->setBoundingSphere(sphere);
...
int PauseMenu::MenuFrameDead(ViewFrustum f, int mouseX, int mouseY, FlyGame* entry)
{
	if(this->currentRender)
		this->currentRender->Update();
	...
	if((448.0f/1920.0f)*this->windowWidth < mouseX && mouseX < (881.0f/1920.0f)*this->windowWidth && (948.0f/1080.0f)*this->windowHeight < mouseY && mouseY < this->windowHeight)
	{
		this->currentRender = this->models[7];
		if(!hover)
			entry->GetCoreInstance()->Audio_PlaySound(FlySound_LevelMenuHover); 
	...
	{
		this->currentRender = this->models[8];
		if(!hover)
			entry->GetCoreInstance()->Audio_PlaySound(FlySound_LevelMenuHover); 
	...
	{
		this->currentRender = this->models[6];
		hover = false; 
	}
